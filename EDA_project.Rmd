...
```{r echo=FALSE, message=FALSE, warning=FALSE}
cast_data1$gender[cast_data1$gender == 0] <- "Unspecified"
cast_data1$gender[cast_data1$gender == 1] <- "Female"
cast_data1$gender[cast_data1$gender == 2] <- "Male"


cast_data11 <- cast_data1 %>% filter(gender != "Unspecified")

ggplot(cast_data11 %>% filter(job == "Director" | job == "Screenplay" | job == "Producer" | job == "Executive Producer" | job == "Editor"), aes(factor(job))) +
  geom_bar(aes(fill = gender)) +
  labs(title = "Gender Distribution by Job", x = "Gender", y = "Count")
...
kable(metascrubdf[maxb,c(1,4,5)], type = "Markdown")
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
maxp <- which.max(metascrubdf$profitM)
kable(metascrubdf[maxp,c(1,4,5)], type = "Markdown")
```
